    ...
    for index in range(len(form["type_product"])):
        if form["type_product"][index] == 'None':
            ...
            try:
                ...
                db.session.rollback()
                Stock.query.filter(
                    Stock.name_product == form["name_product"][index]
                    ).update(
                        {
                            Stock.amount_product:
                            int(
                                form["amount_product"][index]
                                ) + Stock.amount_product
...
@login_required
def get_choice_suitable_product():
    if request.method == 'POST':
        choice_type_product = str(request.data)[2:-1]
        product = get_the_right_product(choice_type_product)
